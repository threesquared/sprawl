...
const CrawlInfo: React.FC<{ pubs: Pub[], start?: LatLng, end?: LatLng }> = ({ pubs, start, end }) => {
  ...
  return (
    <div className="crawlInfo">
      ...
      <small>{ pubs.length <= 10 && (<a href={ `https://www.google.com/maps/dir/?api=1&travelmode=walking&dir_action=navigate&destination=${dest.location.lat},${dest.location.lng}&waypoints=${waypoints}` } target="_blank" rel="noopener noreferrer">Get directions</a>) }</small> | <small><a href={ `#${saveData}` } target="_blank" rel="noopener noreferrer">Link to this crawl</a></small>
    </div>
  );
}
